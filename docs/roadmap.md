...
| 5 | test: 色変換の出典値 KAT（Machado 2009 行列と独立照合・行列退行検出） | ✅ 完了 | #156 |
...
| 4 | vision: 眼精疲労 / ドライアイ（eye_strain / dry_eye） | ✅ 完了 | #36 |
| 4 | cli: --pipe モード（ffmpeg 連携 JPEG ストリーム処理） | ✅ 完了 | #42 |
| N | vision: metamorphopsia / contrast_sensitivity / detail_loss / teichopsia / flickering_stars | ✅ 完了 | #55–#59 |
| 4 | hearing: misophonia / メニエール / 迷路炎、Experience + Urgency 複合記述子 | ✅ 完了 | #102–#104 |
| 4 | cli: --audio モード（WAV 入出力 + AudioPipeline で聴覚フィルタ） | ✅ 完了 | #105 |
| 5 | shaders: depth_aware_blur GLSL / floaters・flickering_stars の CPU↔GLSL 統一 | ✅ 完了 | #107, #134 |
| — | 監査パス: CLI で 5 フィルタ公開 / 全パラメータを enum に一元化 / vertigo・bppv 代表位相 / fmt | ✅ 完了 (v0.5.0) | B1/B2/S1 |
...
sensus 側のコア（vision + hearing + pipeline + shaders + CLI）は機能完成。残りは
universal-experience（Flutter）との接続のみで、これは Flutter 環境が必要なため別リポで進める。
...
- [ ] `cargo publish` v0.5.0 — Cargo.toml / README / CHANGELOG は 0.5.0 に更新済み。タグ付け + `/publish` は手動
...
- 各フィルタには「こうなったらすぐ病院へ」の医学的注記をドキュメントに付ける。緊急度は `Urgency` enum の 3 段（None / EarlyConsultation ⚠️ / Emergency 🚨）に揃える（docs/overview.md「Medical notes」表が正本）
