...
- **焦点・屈折 (Phase 2 / #4) は disk blur (pillbox) を linear sRGB で適用** — Gaussian は採用しない（defocus の点像は瞳孔 = 円の投影 = circle of confusion であり Gaussian ではない）。strength → 画素半径の換算前提は「pupil 4 mm × |D| ≈ angular blur」「視距離 50 cm / 画像 FOV 30°」「`min(W,H)` 比率で myopia 5% / hyperopia 3.3% / presbyopia 2.5% / astigmatism 2.5% (長軸)」。境界は edge replication、内側は per-row span + horizontal prefix sum で `O(W·H·kernel_height)` に抑える。astigmatism の `axis_deg` は **シャープ方向 (cylinder lens 軸)** を指す医学的慣習で、楕円の長軸 (ボケ方向) は `axis_deg + 90°`。`apply()` ファサードは既定軸 90°、軸を変えたい場合は `vision::astigmatism()` を直接呼ぶ
