...
- **色覚特性（protanopia/deuteranopia/tritanopia）は linear sRGB + Machado 2009 per-severity 11段テーブル**（#165, ADR-0008）— gamma 適用済み sRGB に直接行列を掛けない。strength を 0.1 刻みのテーブルへ `index=strength*10` で解決し、グリッド上は該当行列をそのまま・非グリッド点は隣接 2 エントリを行列要素空間で lerp してから直接適用する（severity=1.0 行列 + blend という旧方式 ADR-0002 は誤差 max~111/255 で廃止）。`achromatopsia` だけは対象外で LMS 経路を捨てて BT.709 photopic luminance（NTSC 用 BT.601 ではない）へ linear blend でグレースケール化する（ADR-0002 のまま）
