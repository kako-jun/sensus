...
- **fix: kako-jun/sensus#127 `vertigo`/`bppv_rotation` の非正方形回転と `vertigo` 周辺 disk blur を GLSL に実装**:
  ...
  - **一致根拠**: 実 `.frag` を 1:1 ミラーする `sim_vertigo_glsl`（aspect 補正回転 + 16tap disk blur）/ `sim_uv_rotation_glsl`（aspect 補正回転）で CPU↔`.frag`↔sim を PSNR 検証（別アルゴリズムのインライン化なし）。非正方形 vertigo 64×32 ≈ 37.2 dB / 32×64、回転+blur の 128×96 ≈ 38.4 dB、bppv 64×32 ≈ 45.1 dB / 32×64 すべて **PSNR ≥ 30 dB**。残る乖離源は pixel 座標 vs pixel-center UV の約 0.5px bilinear サンプリング差と 16tap disk 近似のみ。既存の正方形テスト・strength=0 identity は不変で pass。
