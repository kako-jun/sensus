...
- **feat: protanopia / deuteranopia / tritanopia now resolve intermediate `strength` against the full Machado 2009 per-severity table (11 entries, 0.1 steps) instead of blending toward the severity=1.0 matrix**: 旧実装（ADR-0002）は severity=1.0 行列の結果と原色を `strength` で linear blend する近似で、真の per-severity 行列と最大 ~111/255（tritanopia、mean ~4.7/255）ズレていた。`crates/core/src/vision/color.rs` に `PROTANOMALY_TABLE` / `DEUTERANOMALY_TABLE` / `TRITANOMALY_TABLE`（各 11 段、`table[0]`=単位行列・`table[10]`=既存 severity=1.0 const と一致、Machado 2009 / VIP-Sim `myRecolour.cs` と照合済み）を追加し、`strength` をグリッド上ならそのまま・非グリッド点は隣接 2 エントリを行列要素空間で lerp して解決した行列を直接適用する方式（`resolve_severity_matrix`）に変更。`strength=0.0`（identity）/`strength=1.0`（既存 severity=1.0 結果と byte 一致）は不変。GLSL シェーダ（`protanopia.frag` / `deuteranopia.frag` / `tritanopia.frag`）も CPU 側で解決済みの行列を uniform で受け取り直接適用するよう追従（従来の `uStrength` blend は削除）。`achromatopsia` は対象外で ADR-0002 のまま変更なし。ADR-0002 は本変更（ADR-0008）で protanopia/deuteranopia/tritanopia について supersede（achromatopsia は継続）。KAT (`crates/core/tests/color_kat.rs`) に severity=0.5（テーブルのグリッド点）・0.25（非グリッド点の補間）の新規 cross-check を追加。詳細は ADR-0008・`docs/adr/matrix-provenance.md`（Issue #165）。
