...
- **fix: kako-jun/sensus#124 `starbursts.frag` に放射状擬似レイ（gather 型）を実装し CPU レイマーチングと等価化**:
  ...
  - **Rust `f32::round`（0 から離れる半丸め）に揃えた**: GLSL の `floor(x+0.5)` は +∞ 方向の半丸めで負値の .5 で乖離するため、`.frag` に `roundHalfAwayFromZero` を実装し scatter↔gather の座標を bit 単位で一致させた。`.frag` の precision は `mediump` → `highp`（レイ累積の精度確保）。
  - **CPU 変更なし**: gather は scatter の数学的転置なので CPU 側の定式変更は不要だった（dispersion 色・距離減衰・閾値・additive 合成いずれも CPU の式をそのまま gather で評価）。唯一の乖離源は additive 合成の加算順序（scatter=source 走査順、gather=ray→t 順）に由来する f32 丸めのみ。
  - **一致根拠**: 実 `.frag` の gather ループを 1:1 ミラーする `sim_starbursts_glsl`（別アルゴリズムのインライン化なし）で CPU↔`.frag`↔sim を PSNR 検証。dispersion=0（白）/dispersion=1（虹色）・複数明部・中間 strength・非正方形 64×32 すべて **PSNR ≥ 40dB**（加算順序由来の丸めのみが乖離源）。
